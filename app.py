...
#
# Los formularios no traen el lead_id de Zoho; lo recuperamos por correo.
DT_Leads_fb_form_window = DT_Leads_fb_form_window.copy()

if LEAD_ID_COL is not None:
    DT_Leads_fb_form_window["lead_id_form_zoho"] = (
        DT_Leads_fb_form_window[LEAD_ID_COL]
        .map(normalize_id)
    )
else:
    DT_Leads_fb_form_window["lead_id_form_zoho"] = pd.NA
...
zoho_window_by_email = zoho_window_by_email.rename(
    columns={
        ...
        "Fase": "fase_zoho",
        "Estatus de cierre": "estatus_cierre_zoho",
        "Fecha estimada para resolver": "fecha_estimada_resolver_zoho",
        "Pronóstico": "pronostico_zoho",
...
form_name_col = first_existing_col(
    forms_enriched,
    ["full_name", "name", "Nombre completo", "nombre"]
)

if form_name_col is not None:
    form_name_fallback = (
        forms_enriched[form_name_col]
        .astype("string")
        .str.strip()
        .replace({"": pd.NA, "nan": pd.NA, "None": pd.NA})
    )
else:
    form_name_fallback = pd.Series(
        pd.NA,
        index=forms_enriched.index,
        dtype="string"
    )

forms_enriched["nombre_lead_tabla"] = (
    forms_enriched["nombre_zoho"]
    .astype("string")
    .str.strip()
    .replace({"": pd.NA, "nan": pd.NA, "None": pd.NA})
    .fillna(form_name_fallback)
)
...
Un lift mayor a 1 indica una tasa de avance superior al promedio de ese anuncio y transición.
    ...
    # ========================================================
    # FORMULARIOS POR ETAPA DEL FUNNEL
    # ========================================================

    st.divider()
    st.subheader("Formularios por etapa del funnel")

    st.caption(
        "Los tres datasets de formularios se unifican en una sola base. "
        "El cruce con Zoho se realiza por correo normalizado para recuperar lead_id "
        "y los campos comerciales. Las tablas por etapa son acumulativas."
    )

    forms_detail_base = forms_f.copy()

    forms_detail_base["correo_tabla"] = (
        forms_detail_base["email"]
        .astype("string")
        .str.strip()
    )

    forms_detail_base["anuncio_tabla"] = (
        forms_detail_base["ad_name"]
        .astype("string")
        .str.strip()
    )

    forms_detail_base["lead_id_tabla"] = (
        forms_detail_base["lead_id_form_zoho"]
        if "lead_id_form_zoho" in forms_detail_base.columns
        else pd.NA
    )
    ...
    form_detail_cols = [
        ...
        "correo_tabla",
        "lead_id_tabla",
        "estado_posible_cliente_zoho",
        "fase_zoho",
        "estatus_cierre_zoho",
        "fecha_calificacion_zoho",
        "fecha_estimada_resolver_zoho",
        "pronostico_zoho",
        "importe_zoho",
    ] + QUESTION_COLS

    form_detail_cols = [
        col for col in form_detail_cols
        if col in forms_detail_base.columns
    ]

    form_detail_rename = {
        "anuncio_tabla": "Anuncio",
        "nombre_lead_tabla": "Nombre del lead",
        "correo_tabla": "Correo",
        "lead_id_tabla": "lead_id",
        "estado_posible_cliente_zoho": "Estado de Posible cliente",
        "fase_zoho": "Fase",
        "estatus_cierre_zoho": "Estatus de cierre",
        "fecha_calificacion_zoho": "Fecha de calificación",
        "fecha_estimada_resolver_zoho": "Fecha estimada para resolver",
        "pronostico_zoho": "Pronóstico",
        "importe_zoho": "Importe",
    }

    def build_form_detail_table(mask):
        idx = forms_detail_base.index[mask.fillna(False)]
        out = (
            forms_detail_base.loc[idx, form_detail_cols]
            .rename(columns=form_detail_rename)
            .copy()
        )

        if not out.empty:
            out["_orden_fecha"] = forms_detail_base.loc[idx, "fecha_formulario"].values
            out = (
                out.sort_values("_orden_fecha", ascending=False)
    ...
    stage_masks = {
        "Registros": forms_detail_base["en_zoho"],
        "SQL": forms_detail_base["llego_sql"],
        "En proceso de venta": forms_detail_base["llego_proceso"],
        "Cierre": forms_detail_base["llego_cierre"],
    }

    for stage_name, stage_mask in stage_masks.items():
        st.markdown(f"### {stage_name}")
        stage_table = build_form_detail_table(stage_mask)
        st.caption(f"Formularios: {len(stage_table):,}")
        st.dataframe(
            stage_table,
            use_container_width=True,
            hide_index=True
    ...
    st.markdown("### Todos los formularios")

    st.caption(
        "Incluye todos los formularios de los tres datasets dentro del periodo fijo, "
        "sin importar la etapa. Si el correo no encuentra coincidencia en Zoho, "
        "los campos provenientes de Zoho quedan vacíos."
    )

    all_forms_display = (
        forms_detail_base[form_detail_cols]
        .rename(columns=form_detail_rename)
        .copy()
    )

    if not all_forms_display.empty:
        all_forms_display["_orden_fecha"] = forms_detail_base["fecha_formulario"].values
        all_forms_display = (
            all_forms_display
            .sort_values("_orden_fecha", ascending=False)
            .drop(columns=["_orden_fecha"])
        )

    st.caption(f"Total de formularios: {len(all_forms_display):,}")
...
with tab_explore:
    ...
    # ========================================================
    # FILTRO DE SEMANA EXCLUSIVO DE EXPLORACIÓN
    # ========================================================
    # Este filtro se aplica DESPUÉS de los filtros maestros y afecta
    # únicamente a toda la pestaña Exploración.

    explore_week_options = sorted(
        set(
            leads_f["semana_iso"]
            .dropna()
            .astype(str)
    ...
    selected_explore_weeks = st.multiselect(
        "Semana ISO · Exploración",
        options=explore_week_options,
        default=[],
        placeholder="Todas las semanas",
        help=(
            "Deja vacío para analizar todo el periodo maestro. "
            "Selecciona una o varias semanas para filtrar únicamente esta pestaña."
        ),
        key="explore_week_filter"
    )

    explore_leads = leads_f.copy()
    explore_meta = meta_f.copy()

    if selected_explore_weeks:
        explore_leads = explore_leads[
            explore_leads["semana_iso"].astype(str).isin(selected_explore_weeks)
        ].copy()

        explore_meta = explore_meta[
            explore_meta["semana_iso"].astype(str).isin(selected_explore_weeks)
        ].copy()
    ...
    # Gasto Meta por anuncio para el mismo periodo/filtros de Exploración.
    explore_meta_by_ad = aggregate_meta_ad(
        explore_meta
    )
    ...
    spend_by_ad = (
        ...
        .copy()
        if not explore_meta_by_ad.empty
        else pd.DataFrame(
            columns=[
                "Anuncio",
                "Gasto"
    ...
    for ad_name in explore_ads:
        ...
        ad_base = explore_leads[
            explore_leads["ad_name"].astype(str) == str(ad_name)
        ].copy()
    ...
    st.caption(
        "El gasto se toma de Meta a nivel de anuncio para el mismo periodo seleccionado. "
        "Costo x etapa = Gasto del anuncio / Leads que alcanzaron esa etapa. "
        "Incremento del costo = Costo de la etapa actual - Costo de la etapa anterior."
    )
    ...
    for ad_name in explore_ads:
        ...
        for stage_name, leads_n in stage_counts.items():
            ...
            cost_increase = (
                stage_cost - previous_cost
                if pd.notna(stage_cost) and pd.notna(previous_cost)
                else np.nan
            )
            ...
            acquisition_rows.append(
                {
                    ...
                    "Leads": leads_n,
                    "Gasto": ad_spend,
                    "Costo x etapa": stage_cost,
                    "Incremento del costo": cost_increase,
                }
            )

            if pd.notna(stage_cost):
                previous_cost = stage_cost
    ...
    if acquisition_table.empty:
        ...
        acquisition_display["Incremento del costo"] = acquisition_display[
    ...
    # ========================================================
    # EVOLUCIÓN SEMANAL DEL COSTO POR ETAPA
    # ========================================================

    st.markdown("### Costo x etapa x anuncio x semana")

    st.caption(
        "Evolución semanal del costo para las etapas Registros y SQL. "
        "Cada punto representa Gasto semanal del anuncio / Leads que alcanzaron la etapa en esa semana."
    )
    ...
    for week in weekly_keys:

        week_leads = explore_leads[
            explore_leads["semana_iso"].astype(str) == str(week)
        ].copy()

        week_meta = explore_meta[
            explore_meta["semana_iso"].astype(str) == str(week)
        ].copy()
        ...
        week_spend = aggregate_meta_ad(
            week_meta
        )

        for ad_name in week_ads:

            ad_week_leads = week_leads[
                week_leads["ad_name"].astype(str) == str(ad_name)
            ].copy()

            ad_week_journey = lead_stages(
                ad_week_leads
            )

            ad_week_spend_series = week_spend.loc[
                week_spend["ad_name"].astype(str) == str(ad_name),
                "gasto"
            ]

            ad_week_spend = (
                float(ad_week_spend_series.iloc[0])
                if not ad_week_spend_series.empty
                else 0.0
            )

            weekly_stage_counts = {
                "Registros": len(ad_week_journey["Registros"]),
                "SQL": len(ad_week_journey["SQL"]),
            }

            for stage_name, leads_n in weekly_stage_counts.items():

                weekly_cost_rows.append(
                    {
                        "Semana ISO": week,
                        "Anuncio": ad_name,
                        "Etapa": stage_name,
    ...
    weekly_cost_table = pd.DataFrame(
        weekly_cost_rows
    )

    weekly_cost_chart_data = (
        weekly_cost_table.dropna(
            subset=["Costo x etapa"]
        ).copy()
        if not weekly_cost_table.empty
        else pd.DataFrame()
    )

    if weekly_cost_chart_data.empty:

        st.info(
            "No hay suficientes datos para construir la evolución semanal de costos."
        )

    else:

        weekly_cost_chart_data["Serie"] = (
            weekly_cost_chart_data["Anuncio"].astype(str)
            + " · "
            + weekly_cost_chart_data["Etapa"].astype(str)
        )
        ...
        fig_weekly_cost = px.line(
            ...
            labels={
                ...
                "Costo x etapa": "Costo x etapa (MXN)",
                "Serie": "Anuncio · Etapa",
            }
        )
        ...
        fig_weekly_cost.update_layout(
            ...
            yaxis_title="Costo x etapa (MXN)",
            legend_title="Anuncio · Etapa",
            hovermode="x unified"
        )
